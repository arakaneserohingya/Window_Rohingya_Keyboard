    ...
    if ($Remove) {
        ...
        if ($arch -eq 'ARM64' -and $machine -ne 0xAA64) {
            ...
            if ($machine -ne 0xAA64) {
                throw "Expected an ARM64 Windows DLL on this ARM64 system, found 0x$($machine.ToString('X4'))."
            }
        }
        if ($arch -eq 'AMD64' -and $machine -ne 0x8664) {
            throw "Expected an x64 Windows DLL on this x64 system, found 0x$($machine.ToString('X4'))."
        }
